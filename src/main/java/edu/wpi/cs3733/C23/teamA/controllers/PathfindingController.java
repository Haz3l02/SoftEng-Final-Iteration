...
public class PathfindingController extends ServiceRequestController {
   ...
   * given by user text input Preps graph using prepGraphDB method Prints out the path generated by
   * AStar
   */
  @FXML
  public void generatePathNew() {
    ...
    if (startNodeName.getText().equals("") || endNodeName.getText().equals("")) {
      ...
      // create the graph hashMap where String is Long Name and GraphNode is the node
      ...
      String sName = startNodeName.getText();
      String eName = endNodeName.getText();
      ...
      boolean startValid = hospitalL1.containsKey(sName);
      boolean endValid = hospitalL1.containsKey(eName);

      // check both nodes
      if (!startValid && !endValid) {
        reminder.setText("Both Names entered are invalid. Please try again.");
        reminder.setVisible(true);
      }
      // check start node
      else if (!startValid) {
        reminder.setText("The Start Name isn't valid. Please try again.");
        reminder.setVisible(true);
      }
      // check end node
      else if (!endValid) {
        reminder.setText("The End Name isn't valid. Please try again.");
        reminder.setVisible(true);
      } else {
        // run A*
        ArrayList<GraphNode> path = callAStar(hospitalL1, sName, eName); // makes a call to AStar //
  ...
  /**
   * @param graph a HashMap<String, Node> (normally) generated by prepDFS() from reading .csv files
   * @param startNodeName the ID of the node where the user wants to start from
   * @param endNodeName the ID of the node that the user wants to get to
   * @return the path between startNode and endNode, if it exists. Otherwise, null.
   */
  public static ArrayList<GraphNode> callAStarNew(
      HashMap<String, GraphNode> graph, String startNodeName, String endNodeName) {
    return AStar.traverse(graph.get(startNodeName), graph.get(endNodeName));
  }
