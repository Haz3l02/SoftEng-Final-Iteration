...
public class CoordinateScalar {
    ...
    /**
     * Scales the coordinates down in size from their orignal coordinates by multiplying by
     * a scale factor
     ...
     * @return a double[] of length 2, where the first entry is the new xCoord
     * and the second entry is the new yCoord
     */
     ...
     */
    public static double[] scaleCoordinatesReversed(double xCoord, double yCoord, double scaleFactor) {
        // apply the scale factor to the coordinates and floor them (so they remain a whole number)
        xCoord = (xCoord / scaleFactor);
        yCoord = (yCoord / scaleFactor);
